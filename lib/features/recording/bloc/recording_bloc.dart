...
class RecordingBloc extends Bloc<RecordingEvent, RecordingState> {
  RecordingBloc({
    ...
    on<ResumeOngoingRecording>(_onResumeOngoing);
    // droppable(): the default transformer let a fast double-tap on Start
    // enqueue two events; both await the multi-second one-shot GPS fix, both
    // insert an activity row, and the loser sits in the database as a near-empty
    // orphan until the next cold start's reconciliation auto-ceases it. A
    // genuine second start (after the first completed) is unaffected —
    // droppable() only discards events arriving DURING an in-flight handler.
    on<StartRecording>(_onStart, transformer: droppable());
